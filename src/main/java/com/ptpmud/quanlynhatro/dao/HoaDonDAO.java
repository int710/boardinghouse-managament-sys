...
public class HoaDonDAO {
    ...
    public java.util.Map<String, Double> getRevenueByMonth(int months) {
        ...
        for (int i = months - 1; i >= 0; i--) {
            ...
            double revenue = sumTongTienByMonth(thang, nam);
    ...
    /**
     * Lấy số lượng hóa đơn đã thanh toán và chưa thanh toán
     */
    public java.util.Map<String, Long> getInvoiceStatusCount() {
        java.util.Map<String, Long> result = new java.util.HashMap<>();
        String sql = "SELECT trangThai, COUNT(*) as count FROM HoaDon GROUP BY trangThai";
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    result.put(rs.getString("trangThai"), rs.getLong("count"));
                }
            }
        } catch (SQLException ex) { ex.printStackTrace(); }
        return result;
