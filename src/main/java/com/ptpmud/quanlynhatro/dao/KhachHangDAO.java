...
public class KhachHangDAO {
    ...
    public boolean update(KhachHang k) {
        ...
        UPDATE KhachHang SET 
            tenKhachHang=?, soCccd=?, soDienThoai=?, email=?, queQuan=?, ngheNghiep=?, 
    ...
    """;
        ...
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            ps.setString(5, k.getQueQuan());
            ps.setString(6, k.getNgheNghiep());
            ps.setDate(7, k.getNgaySinh());
            ps.setString(8, k.getGioiTinh());
            ps.setString(9, k.getGhiChu());
            ps.setInt(10, k.getIdKhachHang());
    ...
    public boolean insert(KhachHang kh) {
        String sql = "INSERT INTO KhachHang(tenKhachHang, soCccd, soDienThoai, email, queQuan, ngheNghiep, ngaySinh, gioiTinh, ghiChu) VALUES (?,?,?,?,?,?,?,?,?)";
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, kh.getTenKhachHang());
            ps.setString(2, kh.getSoCccd());
            ps.setString(3, kh.getSoDienThoai());
            ps.setString(4, kh.getEmail());
            ps.setString(5, kh.getQueQuan());
            ps.setString(6, kh.getNgheNghiep());
    ...
    /**
     * Lấy danh sách khách hàng không có hợp đồng đang thuê
     */
    public List<KhachHang> findAvailableCustomers() {
        List<KhachHang> list = new ArrayList<>();
        String sql = "SELECT k.* FROM KhachHang k " +
                     "WHERE k.idKhachHang NOT IN (" +
                     "  SELECT h.idKhachHang FROM HopDongThue h WHERE h.trangThai = 'dangThue'" +
